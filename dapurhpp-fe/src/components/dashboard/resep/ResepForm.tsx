          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} noValidate>
            <div className="grid lg:grid-cols-5 gap-6">
              <div className="lg:col-span-2 space-y-5">
                <div className="bg-white rounded-[24px] border border-[#DDC1AE] p-5 shadow-[0_8px_30px_rgba(109,76,65,0.08)]">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold uppercase tracking-[0.1em] text-[#5D4037] ml-2">Nama Resep</label>
                      ...
                      />
                      {errors.nama && <p className="mt-1 flex items-center gap-1 text-xs text-[#BA1A1A] ml-2"><X className="w-3 h-3" /> {errors.nama.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold uppercase tracking-[0.1em] text-[#5D4037] ml-2">Hasil / Batch (pcs)</label>
                      <input type="number" {...register("estimasiHasil", { valueAsNumber: true })}
                      ...
                      />
                      {errors.estimasiHasil && <p className="mt-1 flex items-center gap-1 text-xs text-[#BA1A1A] ml-2"><X className="w-3 h-3" /> {errors.estimasiHasil.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold uppercase tracking-[0.1em] text-[#5D4037] ml-2">Harga Jual (Opsional)</label>
                      <div className="relative">
                        <input type="number" {...register("hargaJual", { valueAsNumber: true })}
                        ...
                        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                          <span className="font-[var(--font-roboto-mono)] text-sm text-[#8D6E63]">Rp</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold uppercase tracking-[0.1em] text-[#5D4037] ml-2">Catatan (Opsional)</label>
                      <textarea {...register("catatan")} rows={3}
                ...
                {/* Foto Resep */}
                <div className="bg-white rounded-[24px] border border-[#DDC1AE] p-5 shadow-[0_8px_30px_rgba(109,76,65,0.08)]">
                  <h3 className="font-[var(--font-playfair)] font-bold text-lg text-[#2A1711] mb-4">Foto Resep</h3>
                  <label className="group relative block w-full aspect-[4/3] rounded-2xl border-2 border-dashed border-[#D9C4B1] bg-[#FFF8F6] overflow-hidden cursor-pointer hover:border-[#FF8A00] hover:bg-[#FFF0E8] transition-all">
                    ...
                    {selectedFile ? (
                      ...
                      <img src={initialData.fotoUrl} alt={initialData.nama} className="w-full h-full object-cover" />
                    ) : (
                      <div className="flex flex-col items-center justify-center w-full h-full gap-2 text-[#8A7362]">
                        <Camera className="w-10 h-10 group-hover:text-[#FF8A00] transition-colors" strokeWidth={1.5} />
                        <p className="text-sm font-[var(--font-be-vietnam)] font-medium group-hover:text-[#FF8A00] transition-colors">Klik untuk upload foto</p>
                        <p className="text-xs text-[#BCAAA4]">JPEG, PNG, WebP max 2MB</p>
                      </div>
                    )}
                    {selectedFile && (
              ...
              <div className="lg:col-span-3 space-y-4">
                <div className="bg-white rounded-[24px] border border-[#DDC1AE] p-5 shadow-[0_8px_30px_rgba(109,76,65,0.08)]">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-[var(--font-playfair)] font-bold text-lg text-[#2A1711]">Bahan yang Digunakan</h3>
                    ...
                    ><Plus className="w-4 h-4" strokeWidth={2.5} /> Tambah Bahan</button>
                  </div>

                  {bahanLoading ? (
                    <div className="flex items-center justify-center py-8"><Loader2 className="w-6 h-6 text-[#FF8A00] animate-spin" /></div>
                  ) : fields.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      <ChefHat className="w-10 h-10 text-[#DDC1AE] mb-2" strokeWidth={1.5} />
                      <p className="text-sm text-[#564334] mb-1">Belum ada bahan</p>
                      <p className="text-xs text-[#8A7362]">Klik "Tambah Bahan" untuk memulai</p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-[#FFF8F6] border-b border-[#DDC1AE]">
                            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334] w-8">NO</th>
                            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334] min-w-[190px]">BAHAN</th>
                            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334] w-28">QTY</th>
                            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334] w-28">HARGA SAT</th>
                            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334] w-32">TOTAL</th>
                            <th className="px-3 py-3 w-8" />
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#F5E6D8]">
                          {fields.map((field, index) => {
                            const selectedBahan = bahanList.find((b) => b.id === watchedDetails?.[index]?.bahanBakuId);
                            const qty = Number(watchedDetails?.[index]?.jumlah) || 0;
                            const hargaSat = Number(selectedBahan?.hargaTerakhir) || 0;
                            const total = qty * hargaSat;

                            return (
                              <tr key={field.id} className="hover:bg-[#FFF8F6] transition-colors">
                                <td className="px-3 py-3 text-[#8A7362] font-[var(--font-roboto-mono)] text-xs">{index + 1}</td>
                                <td className="px-3 py-3">
                                  <div className="flex items-center gap-2.5 min-w-[190px]">
                                    {selectedBahan?.fotoUrl ? (
                                      <img src={selectedBahan.fotoUrl} alt={selectedBahan.nama} className="w-9 h-9 rounded-lg object-cover shrink-0" />
                                    ) : (
                                    ...
                                    <select
                                      value={watchedDetails?.[index]?.bahanBakuId || ""}
                                      onChange={(e) => {
                                        const val = parseInt(e.target.value);
                                        handleBahanChange(index, val);
                                        setValue(`detailResep.${index}.bahanBakuId`, val, { shouldValidate: true });
                                      }}
                                ...
                                <td className="px-3 py-3">
                                  <div className="flex flex-col items-center gap-1">
                                    <input type="number"
                                      {...register(`detailResep.${index}.jumlah`, { valueAsNumber: true })}
                                ...
                                <td className="px-3 py-3">
                                  {hargaSat > 0 ? (
                                    ...
                                    <span className="text-xs text-[#BA1A1A]">Isi harga dulu</span>
                                  )}
                                </td>
                                <td className="px-3 py-3">
                                ...
                                <td className="px-3 py-3">
                                  ...
                                  ><Trash2 className="w-4 h-4" strokeWidth={1.75} /></button>
                                </td>
                              </tr>
                            );
                          })}
                ...
                <div className="bg-[#FFF8F6] rounded-[24px] border border-[#DDC1AE] p-6 shadow-[0_8px_30px_rgba(109,76,65,0.08)]">
                  <div className="grid grid-cols-3 gap-6">
                    <div className="p-4">
                      <p className="text-sm text-[#8A7362] font-[var(--font-be-vietnam)] mb-2">Total Modal / Batch</p>
                      <p className="font-[var(--font-roboto-mono)] font-bold text-2xl text-[#2A1711]">
                        Rp {totalModal.toLocaleString("id-ID")}
                      </p>
                      <p className="text-[11px] text-[#8A7362] mt-1">= jumlah tiap bahan × harga satuan terakhir</p>
                    </div>
                    <div className="p-4">
                      <p className="text-sm text-[#8A7362] font-[var(--font-be-vietnam)] mb-2">Hasil / Batch</p>
                      <p className="font-[var(--font-roboto-mono)] font-bold text-2xl text-[#2A1711]">
                        {Number(estimasiHasil) || 0} pcs
                      </p>
                    </div>
                    <div className="bg-white rounded-xl p-5 border border-[#FF8A00]/20 shadow-sm">
                      <p className="text-sm text-[#FF8A00] font-[var(--font-be-vietnam)] font-semibold mb-2">HPP / pcs</p>
            ...
            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-[#F5E6D8]">
              ...
              >
                {isLoading || uploading ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
                {uploading ? "Mengupload..." : isEdit ? "Simpan Perubahan" : "Simpan Resep"}
              </button>
            </div>
          </form>
        </div>

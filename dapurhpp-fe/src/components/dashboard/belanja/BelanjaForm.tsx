...
const detailSchema = z.object({
  ...
  jumlah: z.number().min(0.001, "Minimal 0.001"),
  satuan: z.string(),
...
const formSchema = z.object({
  ...
  detailBelanja: z.array(detailSchema).min(1, "Minimal 1 item"),
});
...
const totalBelanja =
  watchedDetails?.reduce((sum, d) => sum + (d.totalHarga || 0), 0) || 0;
...
const handleBahanChange = (index: number, bahanId: number) => {
  ...
  if (bahan) {
    ...
    setValue(
      `detailBelanja.${index}.totalHarga`,
      Math.round(bahan.hargaTerakhir * jumlahSekarang),
    );
  }
};
  ...
  const handleJumlahChange = (index: number, jumlah: number) => {
    ...
    if (detail?.bahanBakuId) {
      ...
      if (bahan) {
        setValue(
          `detailBelanja.${index}.totalHarga`,
          Math.round(bahan.hargaTerakhir * jumlah),
        );
      }
    }
  };
    ...
    try {
      await api.post("/belanja", {
        ...
        detailBelanja: data.detailBelanja.map((d) => ({
          bahanBakuId: Number(d.bahanBakuId),
      ...
      <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-[2rem] shadow-[0_24px_64px_-12px_rgba(42,23,17,0.4)]">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-full hover:bg-[#FFF8F6]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <div className="relative max-w-xs">
                <input
                  type="date"
                  {...register("tanggal")}
                  className="w-full pl-10 pr-4 py-3 bg-white border-2 border-[#D9C4B1] rounded-full text-[#2A1711]"
                />
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-[#2A1711]">
                  Daftar Item Belanja
                </h3>
                <button
                  type="button"
                  onClick={addItem}
                  className="flex items-center gap-1 px-4 py-2 rounded-full border border-[#FF8A00] text-[#FF8A00] text-sm font-medium hover:bg-[#FFF8F6]"
                >
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#FFF8F6] border-b border-[#DDC1AE]">
                      <th className="px-3 py-3 text-left text-xs font-semibold text-[#564334]">
                        NO
                      </th>
                      <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-[#564334]">
                        GAMBAR
                      </th>
                      <th className="px-3 py-3 text-left text-xs font-semibold text-[#564334]">
                        BAHAN
                  ...
                  <tbody className="divide-y divide-[#F5E6D8]">
                    {fields.map((field, index) => {
                      ...
                      const hargaPerGram =
                        detail?.jumlah && detail?.jumlah > 0
                          ? Math.round(
                              (detail?.totalHarga || 0) / detail.jumlah,
                            )
                          : 0;

                      return (
                        <tr key={field.id}>
                          <td className="px-3 py-3 text-[#564334]">
                            {index + 1}
                          </td>
                          <td className="px-3 py-3">
                            {/* 2. Gunakan fungsi getImageUrl di sini */}
                            {selectedBahan?.fotoUrl ? (
                              <img
                                src={getImageUrl(selectedBahan.fotoUrl)}
                                alt={selectedBahan.nama}
                                className="w-9 h-9 rounded-lg object-cover bg-white"
                                onError={(e) => {
                                  // Fallback kalau gambar gagal diload/hilang dari backend
                            ...
                            {/* Icon fallback akan muncul jika tidak ada fotoUrl, atau jika loading gambar error */}
                          ...
                          <td className="px-3 py-3">
                            <select
                              {...register(
                                `detailBelanja.${index}.bahanBakuId`,
                                { valueAsNumber: true },
                              )}
                              onChange={(e) =>
                                handleBahanChange(index, Number(e.target.value))
                              }
                              className="w-40 px-3 py-2 rounded-lg border border-[#DDC1AE] text-sm"
                            >
                          ...
                          <td className="px-3 py-3">
                            <select
                              {...register(
                                `detailBelanja.${index}.supplierId`,
                                { valueAsNumber: true },
                              )}
                              className="w-36 px-3 py-2 rounded-lg border border-[#DDC1AE] text-sm"
                            >
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-1">
                              <input
                                type="number"
                                step="0.001"
                                {...register(`detailBelanja.${index}.jumlah`, {
                                  valueAsNumber: true,
                                })}
                                onChange={(e) => {
                                  const val = Number(e.target.value);
                                  setValue(
                                    `detailBelanja.${index}.jumlah`,
                                    val,
                                  );
                                  handleJumlahChange(index, val);
                                }}
                                className="w-24 px-3 py-2 rounded-lg border border-[#DDC1AE] text-sm"
                                placeholder="0"
                              />
                              <span className="text-xs text-[#8A7362] min-w-[40px]">
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                className="w-28 px-3 py-2 rounded-lg border border-[#DDC1AE] text-sm"
                                placeholder="0"
                              />
                              {hargaPerGram > 0 && (
                          ...
                          <td className="px-3 py-3">
                            <button
                              ...
                              className="p-1.5 rounded-full hover:bg-[#FEE2E2] text-[#EF4444]"
                            >
            ...
            <div className="flex items-center justify-between pt-4 border-t border-[#F5E6D8]">
              <div className="text-xs text-[#8A7362]">
                Harga per satuan dihitung otomatis
              </div>
              <div className="text-right">
                <p className="text-xs text-[#8A7362] mb-1">TOTAL BELANJA</p>
                <p className="font-[var(--font-roboto-mono)] font-bold text-3xl text-[#FF8A00]">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-6 py-3 rounded-full bg-[#FF8A00] text-white font-semibold hover:bg-[#E67E00] disabled:opacity-50"
              >
                {isSubmitting ? "Menyimpan..." : "Simpan Belanja"}
              </button>
            </div>
          </form>
        </div>

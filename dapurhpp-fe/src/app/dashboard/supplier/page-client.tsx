...
export default function SupplierPageClient() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      toast.error(t("suppliers.errorLoad"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      if (editTarget) {
        ...
        toast.success(t("suppliers.successCreate"));
      }
      handleCloseForm();
      fetchList();
    } catch (err: any) {
      alert(err.response?.data?.message || t("suppliers.errorCreate"));
      toast.error(t("suppliers.errorCreate"));
      throw err;
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(t("suppliers.errorDelete"));
      setDeleteTargetId(null);
    }
  };
    ...
    <div className="mx-auto max-w-[1500px]">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[#564334] text-lg">{t("suppliers.subtitle")}</p>
        </div>
        <button
        ...
        {isLoading ? (
          <div className="p-6">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse flex items-center gap-3 py-3 border-b border-[#F5E6D8]"
              >
          ...
          <div className="text-center py-16">
            <Truck
              className="w-16 h-16 text-[#DDC1AE] mx-auto mb-4"
              strokeWidth={1.5}
            />
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-[#F5E6D8] flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-sm text-[#564334] font-[var(--font-be-vietnam)]">
                  ...
                  {t("common.pagination.of")} {filteredList.length} supplier
                </p>
